...
export default function Dashboard() {
  ...
  const sendPrompt = async () => {
    ...
    setLoading(true);
    try {
      // Send prompt to your backend AI endpoint. Replace the URL if needed.
      ...
      // Expecting { reply: '...' } from the server. Fallback to data.text if different.
      ...
      if (reply) {
        ...
        // If response shape is unexpected, fallback to a helpful message
        const fallback = { from: 'ai', text: `Sorry, I couldn't understand the AI response.` };
        setMessages((m) => [...m, fallback]);
      }
    } catch (err) {
      // Network or server error — fallback to a local mock response so UI remains functional
